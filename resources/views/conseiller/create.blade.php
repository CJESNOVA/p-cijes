<x-app-layout title="Prescrire une prestation" is-sidebar-open="true" is-header-blur="true">
    <main class="main-content w-full px-[var(--margin-x)] pb-8">
        <div class="flex items-center space-x-4 py-5 lg:py-6">
          ...
          <!-- <ul class="hidden flex-wrap items-center space-x-2 sm:flex">
            ...
            <li>Prescrire une prestation</li>
          </ul> -->
        </div>
        ...
        <form action="{{ route('conseiller.store') }}" method="POST">
            @csrf
                                ...
                                <select name="prestation_id"
                                    ...
                                    @foreach($prestations as $p)
                                        <option value="{{ $p->id }}" {{ old('prestation_id') == $p->id ? 'selected' : '' }}>
                                            {{ $p->titre }} - {{ $p->prestationtype->titre ?? 'Sans type' }} 
                                            - {{ $p->prix }} FCFA - {{ $p->duree }}
                                        </option>
                                    @endforeach
                                </select>
