<x-app-layout title="Modifier la question" is-sidebar-open="true" is-header-blur="true">
<main class="main-content w-full px-[var(--margin-x)] pb-8">
        <div class="flex items-center space-x-4 py-5 lg:py-6">
          <h2
            class="text-xl font-medium text-slate-800 dark:text-navy-50 lg:text-2xl"
          >
            Modifier la question : {{ $quizquestion->titre }}
          </h2>
          ...
          <!-- <ul class="hidden flex-wrap items-center space-x-2 sm:flex">
            ...
            <li>Modifier la question : : {{ $quizquestion->titre }}</li>
          </ul> -->
        </div>
        <div class="grid grid-cols-12 lg:gap-6">
            <div class="col-span-12 pt-6 lg:col-span-8 lg:pb-6">

            <form action="{{ route('quizquestion.update', [
        'quiz' => $quiz->id,
            ...
            >
                <div class="mb-4">
                    ...
                    <input type="text" name="titre"
                        ...
                        value="{{ old('titre', $quizquestion->titre) }}" required>
                </div>
                    ...
                    <select name="quizquestiontype_id"
                        ...
                        @foreach($types as $type)
                            <option value="{{ $type->id }}"
                                {{ old('quizquestiontype_id', $quizquestion->quizquestiontype_id) == $type->id ? 'selected' : '' }}>
                                {{ $type->titre }}
                            </option>
                        @endforeach
                    </select>
