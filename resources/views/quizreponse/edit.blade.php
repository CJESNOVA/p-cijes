<x-app-layout title="Modifier Réponse" is-sidebar-open="true" is-header-blur="true">
<main class="main-content w-full px-[var(--margin-x)] pb-8">
        <div class="flex items-center space-x-4 py-5 lg:py-6">
          <h2
            class="text-xl font-medium text-slate-800 dark:text-navy-50 lg:text-2xl"
          >
            Modifier la réponse de la question : {{ $quizquestion->titre }}
          </h2>
          ...
          <!-- <ul class="hidden flex-wrap items-center space-x-2 sm:flex">
            ...
            <li>Modifier la réponse de la question : {{ $quizquestion->titre }}</li>
          </ul> -->
        </div>
        <div class="grid grid-cols-12 lg:gap-6">
            <div class="col-span-12 pt-6 lg:col-span-8 lg:pb-6">

            <form action="{{ route('quizreponse.update', [$quizquestion, $quizreponse]) }}" method="POST">
                @csrf
                @method('PUT')
            ...
            >
                <div class="mb-4">
                    ...
                    <textarea name="text" class="form-input w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2
                                  ...
                                  dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent" required>{{ old('text', $quizreponse->text) }}</textarea>
              </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="correcte" class="form-select mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2
                        ...
                        <option value="0" {{ old('correcte', $quizreponse->correcte) == "0" ? 'selected' : '' }}>❌ Non</option>
                        <option value="1" {{ old('correcte', $quizreponse->correcte) == "1" ? 'selected' : '' }}>✅ Oui</option>
                    </select>
                </div>
