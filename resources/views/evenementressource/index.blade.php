<x-app-layout title="Ressources utilisées pour les événements" is-sidebar-open="true" is-header-blur="true">
    <main class="main-content w-full px-[var(--margin-x)] pb-8">
        ...
        <div class="mb-10">
            <div class="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-slate-800 dark:text-navy-50">Ressources utilisées pour les événements</h1>
                    <p class="mt-2 text-slate-600 dark:text-navy-200 text-lg">Suivi des transactions pour les inscriptions aux événements</p>
                </div>
            </div>
        ...
        <div class="card shadow-xl">
            <div class="card-header border-b border-slate-200 dark:border-navy-500 px-6 py-4">
                <h3 class="text-xl font-semibold text-slate-800 dark:text-navy-50">Historique des transactions événementielles</h3>
            </div>
            ...
            <div class="card-body p-0">
                @if($ressources->isEmpty())
                    <div class="flex flex-col items-center justify-center py-16">
                        ...
                        <p class="text-slate-600 dark:text-navy-200 text-center max-w-md text-lg">Les transactions pour les inscriptions aux événements apparaîtront ici une fois que des participants s'inscriront.</p>
                    </div>
                @else
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            <thead class="bg-slate-50 dark:bg-navy-800">
                                <tr>
                                    <th class="px-8 py-4 text-left text-xs font-medium text-slate-500 dark:text-navy-200 uppercase tracking-wider">Événement</th>
                                    ...
                                    <tr class="hover:bg-slate-50 dark:hover:bg-navy-700 transition-colors">
                                        <td class="px-8 py-5">
                                            <div class="flex items-start">
                                                <div class="h-10 w-10 bg-9333EA/10 rounded-xl flex items-center justify-center mr-4 flex-shrink-0">
                                                    <svg class="w-5 h-5 text-9333EA" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                                    </svg>
                                                </div>
                                                <div class="min-w-0 flex-1">
                                                    <div class="text-sm font-semibold text-slate-900 dark:text-navy-50 break-words">{{ $res->evenement->titre ?? 'N/A' }}</div>
                                                    <div class="text-xs text-slate-500 dark:text-navy-200">{{ $res->evenement->evenementtype->titre ?? '' }}</div>
                                                </div>
                                            </div>
                                        </td>
