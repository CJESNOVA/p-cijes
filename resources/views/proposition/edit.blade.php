<x-app-layout title="Modifier ma proposition" is-sidebar-open="true" is-header-blur="true">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div>
                                ...
                                <span class="text-slate-800 dark:text-white ml-2">
                                    {{ $proposition->plan->objectif ? Str::limit($proposition->plan->objectif, 100) : 'Non défini' }}
                                </span>
                            </div>
                            <div>
                                <span class="font-medium text-slate-600 dark:text-slate-300">Entreprise:</span>
                                <span class="text-slate-800 dark:text-white ml-2">
                                    {{ $proposition->plan->accompagnement->entreprise->nom ?? 'Non spécifiée' }}
                                </span>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="md:col-span-2">
                                <label class="block">
                                    ...
                                    <textarea name="message" rows="4" 
                                            ...
                                            placeholder="Décrivez votre proposition, vos compétences, votre approche...">{{ old('message', $proposition->message) }}</textarea>
                                ...
                                <label class="block">
                                    ...
                                    <select name="prestation_id" 
                                        ...
                                        @foreach($prestations as $prestation)
                                            <option value="{{ $prestation->id }}" {{ old('prestation_id', $proposition->prestation_id) == $prestation->id ? 'selected' : '' }}>
                                                {{ $prestation->titre }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <label class="block">
                                    ...
                                    <input type="number" name="prix_propose" step="0.01" min="0"
                                            class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2
                                                   placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary
                                                   dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                            placeholder="Ex: 50000" value="{{ old('prix_propose', $proposition->prix_propose) }}">
                                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                                ...
                                <label class="block">
                                    ...
                                    <input type="number" name="duree_prevue" min="1" max="365"
                                            class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2
                                                   placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary
                                                   dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                            placeholder="Ex: 30" value="{{ old('duree_prevue', $proposition->duree_prevue) }}">
                                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                                ...
                                <label class="block">
                                    ...
                                    <input type="date" name="date_expiration"
                                            class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2
                                                   placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary
                                                   dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                            value="{{ old('date_expiration', $proposition->date_expiration) }}"
