<x-app-layout title="Plans d'accompagnement" is-sidebar-open="true" is-header-blur="true">
    <main class="main-content w-full px-[var(--margin-x)] pb-8">
        ...
        <div class="grid grid-cols-12 lg:gap-6">
          ...
          <div class="card px-4 pb-4 sm:px-5 mb-6">
            <div class="max-w-xxl">
              <div class="flex items-center justify-between">
                <div>
                  <h3 class="text-lg font-semibold text-slate-800">Diagnostic Entreprise</h3>
                  <p class="text-sm text-slate-500 mt-1">
                    Entreprise : <span class="font-bold text-purple-600">{{ $diagnostic->entreprise->nom ?? 'Non spécifiée' }}</span> | 
              ...
              <div id="plansParModule" class="space-y-6 hidden">
                ...
                <div class="flex items-center justify-between mb-6">
                  ...
                  <div class="text-sm text-gray-600">
                    ...
                    {{ isset($plansParModule) ? count($plansParModule) : 0 }} module{{ isset($plansParModule) && count($plansParModule) > 1 ? 's' : '' }}
                  </div>
                </div>
                ...
                @php
                  ...
                  foreach($diagnostic->accompagnement->plans as $plan) {
                      ...
                      if($plan->plantemplate && $plan->plantemplate->diagnosticmodule_id) {
                          $moduleId = $plan->plantemplate->diagnosticmodule_id;
                ...
                @foreach($plansParModule as $moduleId => $plans)
                  <div class="bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
                    <!-- Header du module cliquable -->
                    <div class="cursor-pointer flex items-center justify-between p-6 rounded-lg hover:bg-purple-100/50 transition-colors duration-200" 
                         onclick="toggleModulePlans('module-{{ $moduleId }}')">
                      <h4 class="text-lg font-semibold text-purple-800 flex items-center">
                        <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center mr-4">
                          <i class="fas fa-cube text-white"></i>
                        </div>
                        {{ $plans[0]->plantemplate->diagnosticmodule->titre }}
                      </h4>
                      <div class="flex items-center space-x-3">
                        <span class="text-sm font-medium text-purple-600 bg-purple-100 px-3 py-2 rounded-full">
                          {{ count($plans) }} plan{{ count($plans) > 1 ? 's' : '' }}
                        </span>
                        <svg id="chevron-module-{{ $moduleId }}" class="w-6 h-6 text-purple-600 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div id="module-{{ $moduleId }}" class="px-6 pb-6">
                      <div class="space-y-4">
                        @foreach($plans as $plan)
                          <div class="bg-white rounded-lg border border-purple-200 p-4 shadow-sm hover:shadow transition-shadow duration-200">
                            <div class="flex justify-between items-start">
                              <div class="flex-1">
                                <div class="flex items-center space-x-3 mb-3">
                                  @if($plan->plantemplate && $plan->plantemplate->diagnosticquestion)
                                    <span class="text-xs bg-pink-100 text-pink-800 px-3 py-1 rounded-full font-medium">
                                      <i class="fas fa-question-circle mr-1"></i>Q{{ $plan->plantemplate->diagnosticquestion->position }}
                                    </span>
                                  @endif
            ...
            <div class="max-w-xxl">
              <div class="flex flex-col lg:flex-row gap-4 mb-6">
                ...
                <div class="flex gap-2">
                  <select id="filterModule" class="px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-purple-500">
                    <option value="">Tous les modules</option>
                    @php
                      $modulesList = [];
                      if($diagnostic->accompagnement && $diagnostic->accompagnement->plans) {
                        foreach($diagnostic->accompagnement->plans as $plan) {
                          if($plan->plantemplate && $plan->plantemplate->diagnosticmodule) {
                            $modulesList[$plan->plantemplate->diagnosticmodule->id] = $plan->plantemplate->diagnosticmodule->titre;
                          }
                        }
                  ...
                  <select id="filterStatus" class="px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-purple-500">
                  ...
                  <select id="sortBy" class="px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-purple-500">
          ...
          <!-- Liste des plans avec pagination -->
            ...
            <div class="max-w-xxl">
              <div class="flex items-center justify-between mb-6">
                <h3 class="text-lg font-semibold text-slate-800">
                  ...
                  Plans d'accompagnement 
                  <span class="badge badge-purple badge-sm ml-2" id="visibleCount">{{ $diagnostic->accompagnement ? $diagnostic->accompagnement->plans->count() : 0 }}</span>
                </h3>
                <div class="flex space-x-2">
                  <button onclick="afficherTousLesPlans()" class="px-3 py-1 bg-purple-500 text-white rounded text-sm hover:bg-purple-600 transition-colors" id="btnTousPlans">
                    <i class="fas fa-list mr-1"></i>Tous les plans
                  </button>
                  <button onclick="afficherPlansParModule()" class="px-3 py-1 bg-purple-500 text-white rounded text-sm hover:bg-purple-600 transition-colors" id="btnPlansModule">
              ...
              @if($diagnostic->accompagnement && $diagnostic->accompagnement->plans->count() > 0)
                ...
                <div id="plansContainer" class="space-y-4">
                  @foreach($diagnostic->accompagnement->plans as $plan)
                    <div class="plan-item card border-l-4 border-l-purple-500 bg-slate-50 hover:bg-slate-100 transition-colors" 
                         data-module="{{ $plan->plantemplate->diagnosticmodule_id ?? '' }}"
                        ...
                        <div class="flex justify-between items-start">
                            ...
                            <div class="flex items-center space-x-2 mb-2">
                              @if($plan->plantemplate && $plan->plantemplate->diagnosticmodule)
                                <span class="text-xs bg-purple-100 text-purple-800 px-2 py-1 rounded">
                                  <i class="fas fa-cube mr-1"></i>{{ $plan->plantemplate->diagnosticmodule->titre }}
                                </span>
                              @endif
                              @if($plan->plantemplate && $plan->plantemplate->diagnosticquestion)
                          ...
                          <div class="ml-4 text-right">
                            <div class="text-xs text-slate-500">
                              @if($plan->plantemplate && $plan->plantemplate->diagnosticquestion)
                                Plan pour la question {{ $plan->plantemplate->diagnosticquestion->position }}
                              @else
                                Plan pour le module
                              @endif
                            </div>
              ...
              @else
                <div class="text-center py-12">
                  ...
                  <p class="text-slate-500 mb-6">
                    Commencez par créer des plans d'accompagnement personnalisés basés sur les résultats de votre diagnostic d'entreprise.
                  </p>
                </div>
              @endif
            </div>
                ...
                @php
                    ...
                    foreach($diagnostic->diagnosticresultats as $result) {
                        if($result->diagnosticquestion && $result->diagnosticreponse) {
                            ...
                            if(!isset($scoresParModule[$moduleId])) {
                                $scoresParModule[$moduleId] = [
                            ...
                            $scoresParModule[$moduleId]['score_total'] += $score;
                            $scoresParModule[$moduleId]['nombre_questions']++;
                            $scoresParModule[$moduleId]['score_max_possible'] += 2; // Max 2 points par question
                        }
                    }
...
function showAddPlanModal() {
    console.log('showAddPlanModal called (entreprise)');
    document.getElementById('addPlanModal').classList.remove('hidden');
}

function hideAddPlanModal() {
    console.log('hideAddPlanModal called (entreprise)');
...
document.getElementById('addPlanForm').addEventListener('submit', function(e) {
    console.log('Form submit triggered (entreprise)');
    e.preventDefault();
    
    const formData = new FormData(this);
    formData.append('accompagnement_id', '{{ $diagnostic->accompagnement->id ?? 0 }}');
    
    console.log('FormData (entreprise):', Object.fromEntries(formData));
    console.log('Route (entreprise):', '{{ route("plans.store") }}');
    ...
    .then(response => {
        console.log('Response status (entreprise):', response.status);
        return response.json();
    })
    .then(data => {
        console.log('Response data (entreprise):', data);
        if (data.success) {
            hideAddPlanModal();
            location.reload();
        } else {
    ...
    .catch(error => {
        console.error('Error (entreprise):', error);
    ...
    switch(sortBy) {
        ...
        case 'module':
            filteredPlans.sort((a, b) => {
                const moduleA = a.querySelector('.bg-purple-100')?.textContent || '';
                const moduleB = b.querySelector('.bg-purple-100')?.textContent || '';
...
function afficherPlansParModule() {
    ...
    document.getElementById('btnTousPlans').classList.remove('bg-purple-500', 'ring-2', 'ring-purple-500');
