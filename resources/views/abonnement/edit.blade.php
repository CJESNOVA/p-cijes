<x-app-layout title="Modifier un abonnement" is-sidebar-open="true" is-header-blur="true">
                    ...
                    <div class="max-w-xxl">
                        ...
                        <form method="POST" action="{{ route('abonnement.update', $abonnement->id) }}">
                            ...
                            <div class="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5 lg:gap-6">
                                ...
                                <div>
                                    <label class="block">
                                        <span>Type d'abonnement <span class="text-danger">*</span></span>
                                        <select name="abonnementtype_id" required class="form-select mt-1.5 w-full rounded-lg">
                                            <option value="">Sélectionner un type</option>
                                            @foreach($abonnementtypes as $type)
                                                <option value="{{ $type->id }}"
                                                    {{ old('abonnementtype_id', $abonnement->abonnementtype_id) == $type->id ? 'selected' : '' }}>
                                                    {{ $type->titre }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <label class="block">
                                        ...
                                        <select name="devise" required class="form-select mt-1.5 w-full rounded-lg">
                                            @foreach(['FCFA','XOF','EUR','USD'] as $devise)
                                                <option value="{{ $devise }}"
                                                    {{ old('devise', $abonnement->devise) === $devise ? 'selected' : '' }}>
                                                    {{ $devise }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div>
                                    <label class="block">
                                        ...
                                        <input type="date" name="date_echeance" required
                                            value="{{ old('date_echeance', optional($abonnement->date_echeance)->format('Y-m-d')) }}"
                                            class="form-input mt-1.5 w-full rounded-lg">
                                    </label>
                                </div>
                                ...
                                <div>
                                    <label class="block">
                                        ...
                                        <input type="text" name="mode_paiement"
                                            value="{{ old('mode_paiement', $abonnement->mode_paiement) }}"
                                            class="form-input mt-1.5 w-full rounded-lg">
                                    </label>
                                </div>
                                ...
                                <div class="sm:col-span-2">
                                    <label class="block">
                                        ...
                                        <textarea name="commentaires" rows="4"
                                            class="form-input mt-1.5 w-full rounded-lg">{{ old('commentaires', $abonnement->commentaires) }}</textarea>
                                    </label>
                                </div>
                            </div>
                            ...
                            <div class="bg-slate-50 dark:bg-navy-800 rounded-lg p-4 border mt-6">
                                ...
                                <div class="grid md:grid-cols-3 gap-4 text-sm">
                                    <div>Payé : <strong>{{ number_format($abonnement->montant_paye,2) }} {{ $abonnement->devise }}</strong></div>
                                    <div>Restant : <strong>{{ number_format($abonnement->montant_restant,2) }} {{ $abonnement->devise }}</strong></div>
                                    <div>Statut : <strong>{{ $abonnement->statut_label }}</strong></div>
                                </div>
                            </div>

                            {{-- ACTIONS --}}
                            <div class="flex justify-end space-x-4 mt-6">
                                <a href="{{ route('abonnement.index') }}" class="btn bg-slate-200">
                                    Annuler
                                </a>
